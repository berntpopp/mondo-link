...
from mondo_link.server_manager import create_unified_app
...
def client(monkeypatch: pytest.MonkeyPatch) -> TestClient:
    monkeypatch.setattr(
        settings,
        "allowed_hosts",
        ["localhost", "127.0.0.1", "::1", "mondo-link.genefoundry.org"],
    )
...
def test_fastmcp_344_strict_guard_is_installed(client: TestClient) -> None:
    ...
    response = client.get("/mcp", headers={"Host": "mondo-link.genefoundry.org"})
...
def test_absent_and_configured_origins_are_allowed(client: TestClient) -> None:
    no_origin = client.get("/mcp", headers={"Host": "mondo-link.genefoundry.org"})
    configured_origin = client.get(
        "/mcp",
        headers={
            "Host": "mondo-link.genefoundry.org",
...
def test_untrusted_origin_is_rejected_on_every_route(client: TestClient, path: str) -> None:
    response = client.get(
        path,
        headers={"Host": "mondo-link.genefoundry.org", "Origin": "https://evil.example"},
    )
...
def test_json_environment_allowlists_are_parsed(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(
        "MONDO_LINK_ALLOWED_HOSTS",
        '["localhost","mondo-link.genefoundry.org"]',
    )
    monkeypatch.setenv("MONDO_LINK_ALLOWED_ORIGINS", '["https://genefoundry.org"]')

    configured = ServerSettings(_env_file=None)

    assert configured.allowed_hosts == ["localhost", "mondo-link.genefoundry.org"]
    assert configured.allowed_origins == ["https://genefoundry.org"]


@pytest.mark.parametrize(
    "compose_file", ["docker/docker-compose.yml", "docker/docker-compose.npm.yml"]
)
def test_compose_propagates_allowlist_and_healthcheck_host(compose_file: str) -> None:
    text = Path(compose_file).read_text()
    assert "MONDO_LINK_ALLOWED_HOSTS" in text

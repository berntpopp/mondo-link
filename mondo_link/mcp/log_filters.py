...
OWN loggers, BEFORE the ``ArgValidationMiddleware`` / the Layer-3 protocol backstop
reshape the caller-facing frame. These records reflect caller input into a
log/telemetry sink independent of who the caller is. Examples (all real, this
FastMCP/mcp version):
...
This supersedes the earlier ``fastmcp.server.server``-only prefix filter: the probe
showed the pre-middleware reflection also lands on ``mcp.server.lowlevel.server``,
``fastmcp.server.mixins.mcp_operations``, and the ROOT ``mcp.shared.session`` record
(the malformed-URI ``-32602`), at DEBUG as well as WARNING.
"""
